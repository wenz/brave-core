...
// a loosely-coupled container for components, jobs and other objects that are
...
class BATLedgerContext {
 public:
  // An opaque object representing a unique component key.
  class ComponentKey {
   public:
    ComponentKey();
  ...
  //
  // There are currently two primary Object subtypes: context components and
  // jobs. Context components define a static |kComponentKey| member. Instances
  ...
  // completes, the job object is destroyed.
  //
  // Example:
  ...
  auto StartJob(Args&&... args) {
    ...
    result.Then(base::BindOnce(&BATLedgerContext::OnJobCompleted<CompleteType>,
 ...
 private:
  template <typename T>
  void OnJobCompleted(size_t job_key, const T&) {
    EraseJob(job_key);
  }

  void EraseJob(size_t job_key);

  LedgerClient* ledger_client_;
  LedgerImpl* ledger_impl_ = nullptr;
  std::map<size_t, std::unique_ptr<Object>> components_;
  std::map<size_t, std::unique_ptr<Object>> jobs_;
